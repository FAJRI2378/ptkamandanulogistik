...
const Licenses = () => {
  ...
  const licensesData = [
    { id: 1, name: "SIUP", fullName: "Surat Izin Usaha Perdagangan", description: "Lisensi operasional utama yang mengesahkan PT Kamandanu Jaya Samudera untuk menjalankan aktivitas bisnis secara legal.", imageUrl: siupakImg, badge: "Business License" },
    { id: 2, name: "SIUKAK", fullName: "Surat Izin Usaha Keagenan Awak Kapal", description: "Izin resmi dari Ditjen Perhubungan Laut Kemenhub bagi perusahaan perekrutan/penempatan awak kapal, baik domestik maupun asing yang mengacu pada PM Nomor 59 Tahun 2021.", imageUrl: siukakImg, badge: "Operational Permit" },
    { id: 3, name: "ISO & MLC", fullName: "Quality Management Certifications", description: "Standar internasional untuk manajemen kualitas dan perlindungan tenaga kerja maritim global.", imageUrl: isoImg, badge: "International Standard Management" },
  ];

  const certificationData = [
    { id: 4, name: "Mindset Transformation", fullName: "Mindset & Behavior Transformation", description: "Sertifikasi pengembangan karakter pelaut yang baik untuk memastikan integritas dan performa unggul di laut.", imageUrl: mabt, badge: "Professional Training" },
    { id: 5, name: "Mental Health & Stress Management", fullName: "ITF Stress Management & Coping", description: "Sertifikasi internasional untuk menjaga kesehatan mental dan kesejahteraan kru di atas kapal.", imageUrl: tot, badge: "Professional Training" }
  ];
  ...
  return (
    <>
      ...
      <main className="bg-[#F8FAFC] min-h-screen font-sans selection:bg-blue-100 selection:text-blue-900">
        ...
        {/* --- CONTENT SECTION (FIXED OVERLAP) --- */}
        {/* Menghapus margin negatif yang menyebabkan tabrakan teks di mobile */}
...
const LicenseCard = ({ data }) => (
  <div className="group bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(10,37,64,0.05)] border border-gray-100/50 overflow-hidden hover:shadow-[0_40px_80px_rgba(10,37,64,0.1)] hover:-translate-y-2 transition-all duration-500 flex flex-col h-full">
    <div className="relative h-64 md:h-72 bg-gray-50/50 flex items-center justify-center p-8 overflow-hidden">
      ...
      <img src={data.imageUrl} alt={data.name} className="max-w-full max-h-full object-contain relative z-10 transition-all duration-700 group-hover:scale-110 drop-shadow-xl" />
    ...
    <div className="p-8 md:p-10 flex-grow flex flex-col">
      <h3 className="text-2xl font-black text-[#0A2540] mb-1 group-hover:text-blue-600 transition-colors leading-tight">{data.name}</h3>
      <p className="text-[10px] md:text-[11px] font-black text-blue-500/80 mb-4 uppercase tracking-wider">{data.fullName}</p>
      <p className="text-gray-500 leading-relaxed font-medium text-sm md:text-base mb-8">{data.description}</p>
      <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between gap-2">
        <a href={data.imageUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#0A2540] font-black text-xs md:text-sm group/btn hover:text-blue-600 transition-colors">
          View Document
